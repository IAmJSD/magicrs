...
macro_rules! func {
    ($expr_map:expr, $name:expr, $fn:expr) => {
        $expr_map
            .set_function($name.to_string(), evalexpr::Function::new($fn))
            .unwrap();
    };
}
...
pub fn add_default_functions(expr_map: &mut evalexpr::HashMapContext, body_arc: Arc<Vec<u8>>) {
    // Add json_path.
    let ar = body_arc.clone();
    func!(expr_map, "json_path", move |arg| {
        // Parse the JSON.
        ...
        let path_result = match process_json_path(body, path) {
            ...
            Err(e) => {
                return Err(evalexpr::EvalexprError::CustomMessage(format!(
                    "Failed to process the JSON path: {}.",
                    e
                )))
            }
        };
    ...
    // Add xml_path.
    let ar = body_arc.clone();
    func!(expr_map, "xml_path", move |arg| {
        // Process the body as a string.
        let body = match std::str::from_utf8(&ar) {
            Ok(s) => s,
            Err(e) => {
                return Err(evalexpr::EvalexprError::CustomMessage(format!(
                    "Failed to convert the body to a string: {}.",
                    e
                )))
            }
        };
        ...
        let package = match sxd_document::parser::parse(body) {
            ...
            Err(e) => {
                return Err(evalexpr::EvalexprError::CustomMessage(format!(
                    "Failed to parse the body as XML: {}.",
                    e
                )))
            }
        };
        ...
        let value = match sxd_xpath::evaluate_xpath(&document, &path) {
            ...
            Err(e) => {
                return Err(evalexpr::EvalexprError::CustomMessage(format!(
                    "Failed to evaluate the XML path: {}.",
                    e
                )))
            }
        };
    ...
    // Add base64_encode.
    func!(expr_map, "base64_encode", |arg| {
    ...
    // Add base64_decode.
    func!(expr_map, "base64_decode", |arg| {
        let str = match arg.as_string() {
            Ok(s) => s,
            Err(e) => return Err(e),
        };
        ...
        Ok(evalexpr::Value::String(
            match std::str::from_utf8(&decoded) {
                ...
                Err(e) => {
                    return Err(evalexpr::EvalexprError::CustomMessage(format!(
                        "Failed to convert the base64 string to a string: {}.",
                        e
                    )))
                }
            },
    ...
    // Add string_body.
    func!(expr_map, "string_body", move |_| {
