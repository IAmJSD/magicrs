...
static EMOJIS: Lazy<Vec<&'static str>> =
    Lazy::new(|| emojis::iter().map(|emoji| emoji.as_str()).collect());

// Defines the regex for emoji randoms.
static EMOJI_RANDOM_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"\{random:emoji(:\d+)?\}").unwrap());

// Defines the regex for number randoms.
static NUMBER_RANDOM_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"\{random:(\d+)-(\d+)\}").unwrap());

// Defines the regex for alphabet randoms. This defaults to all lowercase letters.
static ALPHABET_RANDOM_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"\{random:alphabet(:\d+)?\}").unwrap());

// Defines the regex for alphabet randoms with a range of characters.
static ALPHABET_RANDOM_RANGE_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r"\{random:alphabet:([a-zA-Z])-([a-zA-Z])(:\d+)?\}").unwrap());
...
pub fn get_filename(filename_tpl: Option<String>) -> Result<String, String> {
    ...
    filename = filename.replace("{date}", &now.format("%x").to_string().replace("/", "-"));

    // Replace all instances of {time} with the current time in locale format.
    filename = filename.replace("{time}", &now.format("%X").to_string().replace(":", "-"));
    ...
    loop {
        ...
        if let Some(captures) = NUMBER_RANDOM_REGEX.captures(&filename) {
            ...
            if min > max {
                return Err(
                    "Minimum value is greater than maximum value in the filename numeric matcher."
                        .to_string(),
                );
            }
            ...
            filename = NUMBER_RANDOM_REGEX
                .replace(&filename, number.to_string().as_str())
                .to_string();
        ...
        if let Some(captures) = ALPHABET_RANDOM_REGEX.captures(&filename) {
            ...
            let alphabets = (0..count)
                .map(|_| rand::thread_rng().gen_range(b'a'..=b'z') as char)
                .collect::<String>();

            // Replace the alphabet random.
            filename = filename.replacen(captures.get(0).unwrap().as_str(), alphabets.as_str(), 1);
        ...
        if let Some(captures) = ALPHABET_RANDOM_RANGE_REGEX.captures(&filename) {
            ...
            // Get a random set of alphabets.
            let alphabets = (0..count)
                .map(|_| rand::thread_rng().gen_range(min..=max) as char)
                .collect::<String>();
